...
void PrintUciOutput(const int score, const int depth, const S_ThreadData* td, const  S_UciOptions* options) {
	...
	uint64_t nps = nodes / (static_cast<unsigned long long>(time) + !time) * 1000;
	if (print_uci)
	{
		...
		if (score > -mate_value && score < -mate_score) {
			parsed_score = std::abs((score + static_cast<float>(mate_value) / 2));
			score_unit = "-M";
		}
		else if (score > mate_score && score < mate_value) {
			parsed_score = (static_cast<float>(mate_value - score)) / 2 + 1;
			score_unit = "+M";
		}
		else {
